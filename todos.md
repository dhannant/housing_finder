...
- [ ] Add a test script to root and functions package.json for easy test running ("test": "jest").
- [X] Use __mocks__ directory or jest.mock for Firebase/external services to avoid real network calls in tests.
- [X] Add tests for key frontend components and user flows using React Testing Library.
- [X] Add tests for each backend Cloud Function, especially those with business logic or side effects.
- [ ] Set up code coverage reporting with Jest.
- [ ] Set up CI (e.g., GitHub Actions) to run tests automatically on push/PR.

## Dependency Troubleshooting

- If you see an npm ERESOLVE error about React and @testing-library/react, run:
  - npm install --legacy-peer-deps
- This is safe for dev/test tools and is due to React 19 being newer than some peer dependencies.
- Watch for updates to @testing-library/react for React 19 support.
